...
describe('useRecipeDetail', () => {
  ...
  describe('fetchRecipe', () => {
    ...
    it('includes nutrition data in recipe', async () => {
      const { result } = renderHook(() => useRecipeDetail())
      ...
      await waitFor(() => {
        expect(result.current.recipe.nutrition).toBeDefined()
        expect(result.current.recipe.nutrition.nutrients).toBeDefined()
      })
    })

    it('includes ingredients data in recipe', async () => {
      const { result } = renderHook(() => useRecipeDetail())
      ...
      await waitFor(() => {
        expect(result.current.recipe.extendedIngredients).toBeDefined()
        expect(result.current.recipe.extendedIngredients.length).toBeGreaterThan(0)
      })
    })
