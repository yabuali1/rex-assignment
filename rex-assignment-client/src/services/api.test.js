...
describe('recipeApi', () => {
  ...
  describe('getRecipeById', () => {
    ...
    it('includes extended ingredients', async () => {
      const result = await recipeApi.getRecipeById(1)

      expect(result.extendedIngredients).toBeDefined()
      expect(result.extendedIngredients.length).toBeGreaterThan(0)
      expect(result.extendedIngredients[0]).toHaveProperty('id')
      expect(result.extendedIngredients[0]).toHaveProperty('name')
    })
