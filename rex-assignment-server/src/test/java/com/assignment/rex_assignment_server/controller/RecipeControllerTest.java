...
class RecipeControllerTest {
    ...
    class SearchRecipesTests {
        ...
        void shouldReturnRecipesWithFilters() throws Exception {
            // Arrange
            RecipeSearchResponse mockResponse = createMockSearchResponse();
            when(recipeService.searchRecipes(eq("pasta"), eq("vegetarian"), eq("italian"), eq("main course"), anyInt(),
                    anyInt()))
                    .thenReturn(mockResponse);
    ...
    @Nested
    @DisplayName("GET /api/recipes/{id}/exclude")
    class GetRecipeWithExclusionsTests {

        @Test
        @WithMockUser
        @DisplayName("should return 200 and recipe with excluded ingredients")
        void shouldReturnRecipeWithExclusions() throws Exception {
            // Arrange
            RecipeDetailResponse mockResponse = createMockRecipeDetail();
            when(recipeService.getRecipeWithExcludedIngredients(eq(123L), anyList()))
                    .thenReturn(mockResponse);

            // Act & Assert
            mockMvc.perform(get("/api/recipes/123/exclude")
                    .param("excludeIngredients", "pasta")
                    .param("excludeIngredients", "cheese"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.id", is(123)));
        }
        ...
        @Test
        @WithMockUser
        @DisplayName("should return 200 without exclusions when no params provided")
        void shouldReturnRecipeWithoutExclusions() throws Exception {
            // Arrange
            RecipeDetailResponse mockResponse = createMockRecipeDetail();
            when(recipeService.getRecipeWithExcludedIngredients(eq(123L), isNull()))
                    .thenReturn(mockResponse);
        ...
        void shouldReturn404WhenRecipeNotFound() throws Exception {
            // Arrange
            when(recipeService.getRecipeWithExcludedIngredients(eq(999L), any()))
                    .thenThrow(new RecipeNotFoundException("Recipe not found"));

            // Act & Assert
            mockMvc.perform(get("/api/recipes/999/exclude"))
                    .andExpect(status().isNotFound());
        }
    }
